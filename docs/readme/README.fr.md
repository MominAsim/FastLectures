...
<p align="center">
  <a href="https://fastlectures.ai">Site web</a> ·
  <a href="https://github.com/fastlectures/fastlectures/releases/latest">Télécharger</a> ·
...
<p align="center">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_full_demo.webp" alt="Démonstration complète de FastLectures" width="49%">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_plugins.webp" alt="Démonstration de schémas professionnels FastLectures" width="49%">
</p>

<p align="center">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_plugins_sub_x10.webp" alt="Démonstration des extensions FastLectures" width="49%">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/play_patris.webp" alt="Démonstration du canevas interactif FastLectures" width="49%">
</p>
...
Continuez à échanger avec **Codex, Claude, Kimi ou d’autres agents IA**. FastLectures donne une place à votre travail.
...
| --- | --- | --- |
| Travaillez avec l’agent IA que vous utilisez déjà. | Le serveur MCP de FastLectures apporte schémas, documents et aperçus interactifs sur le Canvas. | Essayez le résultat, annotez-le et laissez votre agent lire vos retours pour la prochaine révision. |

<p align="center">
<<<<<<< HEAD
  <a href="../assets/mcp-spatial-example.png">
    <img src="../assets/mcp-spatial-example.png" alt="Une discussion d’architecture avec l’IA, accompagnée de retours manuscrits à côté de la proposition sur un Canvas FastLectures" width="760">
=======
...
| **Cloud MCP** | Connectez des agents IA externes directement à vos Canvas FastLectures Cloud activés pour lire le contenu, créer et modifier des résultats et suivre vos retours manuscrits. Cloud MCP et Local MCP sont des modes de connexion facultatifs. |
| **API FastLectures Cloud Credits** | Utilisez les modèles hébergés par FastLectures avec vos crédits de compte, en complément de vos connexions API et CLI. Consultez les modèles disponibles, les tarifs et le solde dans les paramètres. |
...
<p align="center">
  <img src="../../public/fastlectures-architecture.webp" alt="Architecture FastLectures : un navigateur se connecte à FastLectures Cloud ou à votre PC local. Le Cloud propose des modèles hébergés et se connecte à votre appareil lié. Votre PC exécute FastLectures CLI ou App avec votre API de modèle ou vos agents. Les agents IA externes peuvent utiliser Cloud MCP ou Local MCP ; les deux connexions sont facultatives." width="1483">
</p>

Ouvrez FastLectures dans un navigateur via FastLectures Cloud ou votre PC local exécutant la CLI ou l’application de bureau. Le Cloud propose des modèles hébergés et peut se connecter à votre appareil lié ; votre PC peut utiliser votre propre API de modèle ou vos agents. Des agents IA externes comme Codex et Claude peuvent se connecter via Cloud MCP ou Local MCP. Les deux connexions MCP sont facultatives.

Consultez les [notes d’architecture](../architecture.md) pour les détails d’implémentation.

## Démarrage rapide

**Application de bureau :** téléchargez l’application Windows ou macOS depuis [GitHub Releases](https://github.com/fastlectures/fastlectures/releases/latest).
...
Ouvrez `http://localhost:3888`. Ajoutez votre propre API de modèle ou une CLI Codex, Claude Code ou Kimi authentifiée dans **Paramètres → Connexions**. Les connexions sont enregistrées dans `~/.fastlectures/connections.json` ; les paramètres généraux restent dans `~/.fastlectures/config.env`. Pour les modèles hébergés par FastLectures, connectez-vous et sélectionnez un modèle disponible dans les paramètres.
...
1. Démarrez FastLectures et activez le Canvas actuel dans **Paramètres → Service MCP**.
...
3. Demandez à votre agent : **« Montre l’architecture dont nous avons discuté sur mon Canvas FastLectures. »**

L’agent peut capturer le contenu pertinent, modifier des objets, créer des résultats visuels, appliquer des modifications aux fichiers sources des documents et recevoir vos retours. Seuls les Canvas activés et connectés sont détectables. Avec Local MCP, le client MCP s’exécute sur l’hôte FastLectures ; la prise en charge des navigateurs du réseau local et des appareils liés n’expose pas publiquement le point d’accès MCP local. Cloud MCP est une connexion HTTPS authentifiée distincte pour vos Canvas FastLectures Cloud activés.

Les installations de bureau doivent utiliser la configuration générée, qui inclut le bon environnement d’exécution intégré. Consultez la [configuration MCP](../mcp-setup.md) et la [compétence de workflow pour agents](../../skills/fastlectures-mcp/SKILL.md), facultative.

## FastLectures Cloud et connexions IA

[FastLectures Cloud](https://fastlectures.ai) ajoute des projets privés avec historique des versions, des favoris synchronisés, le partage public via Echoes et l’accès distant à un ordinateur lié.

| Connexion | Fonctionnement |
| --- | --- |
| **Modèles FastLectures** | Connectez-vous, sélectionnez un modèle hébergé disponible et utilisez les crédits de votre compte. Les paramètres affichent les tarifs et le solde actuels. |
...
Vos propres connexions API et CLI ne consomment pas de crédits FastLectures. Un compte Cloud est facultatif pour une utilisation locale avec votre propre connexion. Les fonctions IA nécessitent l’accès au fournisseur sélectionné ; exécuter FastLectures localement ne rend pas un modèle distant disponible hors ligne.

## Configurations de modèles recommandées

Ces recommandations équilibrent la qualité des réponses et la latence dans les tâches réelles de canevas FastLectures, à partir de tests pratiques actuels ; le temps de réponse réel varie selon le fournisseur, la complexité du canevas et le comportement de raisonnement.
...
| [Kimi K3](https://platform.kimi.ai?aff=fastlectures) (`kimi-k3`) | `medium` | Très bonne qualité ; `medium` conserve un équilibre pratique | Réglage Kimi recommandé par défaut |
...
Lisez [CONTRIBUTING.md](../../CONTRIBUTING.md) pour contribuer ; exécutez `npm run check` avant d’ouvrir une pull request. Signalez les bugs dans les [Issues](https://github.com/fastlectures/fastlectures/issues), échangez dans les [Discussions](https://github.com/fastlectures/fastlectures/discussions) ou rejoignez [Discord](https://discord.gg/3jrPJ3mXdX).
