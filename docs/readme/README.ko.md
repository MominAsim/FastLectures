...
<p align="center">
  <a href="https://fastlectures.ai">웹사이트</a> ·
  <a href="https://github.com/fastlectures/fastlectures/releases/latest">다운로드</a> ·
...
<p align="center">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_full_demo.webp" alt="FastLectures 전체 데모" width="49%">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_plugins.webp" alt="FastLectures 전문 다이어그램 데모" width="49%">
</p>

<p align="center">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/fastlectures_plugins_sub_x10.webp" alt="FastLectures 플러그인 데모" width="49%">
  <img src="https://github.com/fastlectures/fastlectures/releases/download/v0.1.0/play_patris.webp" alt="FastLectures 대화형 캔버스 데모" width="49%">
</p>
...
**Codex, Claude, Kimi 등 AI 에이전트**와 대화를 이어 가세요. 작업 결과는 FastLectures에 펼쳐 놓으세요.
...
| **Cloud MCP** | 외부 AI 에이전트를 활성화된 FastLectures Cloud 캔버스에 직접 연결해 콘텐츠를 읽고, 결과를 만들고 편집하고, 손글씨 피드백을 확인합니다. Cloud MCP와 Local MCP는 모두 선택적인 연결 방식입니다. |
| **FastLectures Cloud Credits API** | 자신의 API 및 CLI 연결과 함께 계정 크레딧으로 FastLectures 호스팅 모델을 사용할 수 있습니다. 설정에서 사용 가능한 모델, 요금, 잔액을 확인합니다. |
...
<p align="center">
  <img src="../../public/fastlectures-architecture.webp" alt="FastLectures 구조: 브라우저가 FastLectures Cloud 또는 로컬 PC에 연결됩니다. Cloud는 호스팅 모델을 제공하고 연결된 기기에도 연결할 수 있습니다. PC는 자신의 LLM API 또는 에이전트와 함께 FastLectures CLI나 앱을 실행합니다. 외부 AI 에이전트의 Cloud MCP 및 Local MCP 연결은 모두 선택 사항입니다." width="1483">
</p>

FastLectures Cloud 또는 CLI나 데스크톱 앱을 실행하는 로컬 PC를 통해 브라우저에서 FastLectures를 엽니다. Cloud는 호스팅 모델을 제공하고 연결된 기기에도 접근할 수 있습니다. PC에서는 자신의 모델 API나 에이전트를 사용할 수 있습니다. Codex, Claude 등 외부 AI 에이전트는 Cloud MCP 또는 Local MCP로 연결할 수 있으며, 두 연결 모두 선택 사항입니다.
...
**데스크톱:** [GitHub Releases](https://github.com/fastlectures/fastlectures/releases/latest)에서 Windows 또는 macOS 앱을 다운로드하세요.
...
`http://localhost:3888`을 엽니다. **설정 → 연결**에서 자신의 모델 API 또는 인증된 Codex, Claude Code, Kimi CLI를 추가합니다. 연결은 `~/.fastlectures/connections.json`에 저장되며 일반 설정은 `~/.fastlectures/config.env`에 유지됩니다. FastLectures 호스팅 모델은 로그인 후 설정에서 사용 가능한 모델을 선택하세요.
...
1. FastLectures를 시작하고 **설정 → MCP 서비스**에서 현재 Canvas를 활성화합니다.
...
3. 에이전트에게 요청하세요: **“우리가 논의한 아키텍처를 내 FastLectures Canvas에 보여 줘.”**

에이전트는 관련 콘텐츠를 캡처하고, 객체를 편집하고, 시각적 결과를 만들고, 문서 소스 파일을 패치하고, 피드백을 받을 수 있습니다. 활성화되어 연결된 Canvas만 검색됩니다. Local MCP에서는 MCP 클라이언트가 FastLectures 호스트에서 실행됩니다. LAN 및 연결된 기기의 브라우저 지원은 로컬 MCP 엔드포인트를 공개적으로 노출하지 않습니다. Cloud MCP는 활성화된 FastLectures Cloud 캔버스를 위한 별도의 인증된 HTTPS 연결입니다.

데스크톱 설치에서는 올바른 번들 런타임이 포함된 생성 설정을 사용하세요. [MCP 설정](../mcp-setup.md)과 선택 사항인 [에이전트 워크플로 스킬](../../skills/fastlectures-mcp/SKILL.md)을 참고하세요.

## FastLectures Cloud와 AI 연결

[FastLectures Cloud](https://fastlectures.ai)는 비공개 프로젝트 버전 관리, 즐겨찾기 동기화, Echoes를 통한 공개 공유, 연결된 컴퓨터 원격 접근을 제공합니다.

| 연결 | 작동 방식 |
| --- | --- |
| **FastLectures 모델** | 로그인하고 사용 가능한 호스팅 모델을 선택해 계정 크레딧으로 사용합니다. 설정에 현재 요금과 잔액이 표시됩니다. |
...
자신의 API 및 CLI 연결은 FastLectures 크레딧을 소비하지 않습니다. 자신의 연결로 로컬에서 사용할 때 Cloud 계정은 선택 사항입니다. AI 기능에는 선택한 제공업체에 대한 접근이 필요하며, FastLectures를 로컬에서 실행해도 원격 모델을 오프라인으로 사용할 수는 없습니다.

## 권장 모델 설정

현재의 실제 테스트를 바탕으로 FastLectures의 실제 캔버스 작업에서 답변 품질과 지연 시간의 균형을 고려한 권장 설정입니다. 실제 응답 시간은 제공업체, 캔버스 복잡도, 추론 동작에 따라 달라집니다.
...
| [Kimi K3](https://platform.kimi.ai?aff=fastlectures) (`kimi-k3`) | `medium` | 매우 좋은 품질, `medium`에서 실용적인 균형 유지 | Kimi 권장 기본값 |
...
기여하려면 [CONTRIBUTING.md](../../CONTRIBUTING.md)를 읽고 풀 리퀘스트 전에 `npm run check`를 실행하세요. 버그는 [Issues](https://github.com/fastlectures/fastlectures/issues), 아이디어는 [Discussions](https://github.com/fastlectures/fastlectures/discussions), 커뮤니티 참여는 [Discord](https://discord.gg/3jrPJ3mXdX)를 이용하세요.
