...
- `penecho_start_session`: `{instanceId, canvasId, title, client?, sessionKey?}`. The board baseline is established only after the first session board is successfully applied; the returned `feedbackCursor` is the position after that application.
...
- `penecho_present_widget`: `{sessionId, artifactId, title, html, width?, height?, capture?, quality?}`. HTML is at most 200,000 characters and 800,000 UTF-8 bytes; width is 300–4096 and height 200–4096 when supplied. Keep `artifactId` stable to update one preview. Ordinary presentation defaults to `capture:false`; use `capture:true` only when the model needs immediate visual evidence. `quality` (`basic|detail`) is valid only with `capture:true`. The result's `feedbackCursor` is after the presentation has applied. A captured result includes an image and `pixelVerified:true`.
...
- Preview: with a selected valid connection, proactively present useful UI choices, previews, or a visual explanation when the user benefits. Skip decorative updates for routine edits. Widget choice buttons are not callbacks to this client yet; users indicate a choice in chat or with a Canvas annotation.
...
- Installing the portable PenEcho workflow skill only influences context when the client loads that skill. Installation alone does not guarantee loading into an existing conversation; follow the client’s reload behavior. It does not edit client configuration or make Widget buttons callbacks.
...
- MCP prompts are user-selected templates. This server currently implements no `prompts/list` or `prompts/get`; do not claim that PenEcho added prompts or automatically selected one.
...
- `penecho_start_session`：`{instanceId, canvasId, title, client?, sessionKey?}`。只有首次 session board 成功应用后才建立反馈基线；返回的 `feedbackCursor` 是该应用完成之后的位置。
...
- `penecho_present_widget`：`{sessionId, artifactId, title, html, width?, height?, capture?, quality?}`；HTML 最多 200,000 个字符且最多 800,000 个 UTF-8 字节，width 为 300–4096、height 为 200–4096；同一 session 中保持 `artifactId` 稳定以更新一个预览。普通呈现默认 `capture:false`；只有模型需要即时视觉证据时才设置 `capture:true`。只有 `capture:true` 时 `quality` 才能使用 `basic|detail`。返回的 `feedbackCursor` 位于呈现应用之后；捕获结果带 image 和 `pixelVerified:true`。
...
- 预览：已有有效连接且用户能从中受益时，主动提供有用的 UI 选择、预览或视觉解释；普通编辑跳过装饰性更新。Widget 内的选择按钮暂时不是回调；用户应在聊天中表达选择，或在 Canvas 上添加标注。
...
Skill 与 prompt 的语义：
- 安装便携 PenEcho workflow skill 只会在客户端加载该 skill 时影响上下文；不会激活已运行的对话、修改客户端配置，也不会让 Widget 按钮变成回调。
- stdio server 已经在 `initialize.instructions` 中返回桥接工作流指引。客户端自行决定是否采用、显示或合并这些指引。
- MCP prompts 是由用户选择的模板。当前 server 没有实现 `prompts/list` 或 `prompts/get`；不要声称 PenEcho 新增了 prompts，或自动选择了某个 prompt。
...
create Widget callbacks. The stdio `initialize.instructions` is already present
and the client controls whether to adopt it. MCP prompts are user-selected
templates; this server currently has no `prompts/list` or `prompts/get`.
...
改变上下文；不会修改客户端配置、激活已停止的对话或创建 Widget 回调。stdio 的
`initialize.instructions` 已经存在，是否采用由客户端决定。MCP prompts 是用户选择的
模板；当前 server 没有 `prompts/list` 或 `prompts/get`。
