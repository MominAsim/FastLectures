# FastLectures Agent browser E2E — 2026-08-25

Scope: FastLectures Agent only. Legacy Canvas AI is explicitly excluded.
...
- Fixed: Claude/Kimi usage callbacks were not wired through the FastLectures Agent CLI adapter.
...
- Fixed P0: provider activity/heartbeat could keep a FastLectures Agent model step alive beyond its configured hard total limit. Harness now owns a host-level deadline, records one synthetic `TIMEOUT`, restores the UI to an input-ready state, cancels the underlying provider, and suppresses the later abort duplicate. A real DeepSeek image request with a 30-second total limit ended automatically in 29.031 seconds; trace `1787668664428-7f907c14-e8e0-4664-9b1f-10b17c93c5f2`.
