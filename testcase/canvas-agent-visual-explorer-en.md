# FastLectures Agent and Visual Explorer Test Cases
...
- Start each case in a new FastLectures Agent conversation unless the case says otherwise.
...
- “Observe tool activity” means the visible FastLectures Agent tool cards and status; chain-of-thought is never required.
- Record Local Web, Desktop owner, and Cloud Linked Device runs separately. Cloud Viewer and read-only Mobile surfaces must not expose an available FastLectures Agent.
...
- It creates one `general/html_widget` with `sourceFormat` `fastlectures-visual-explorer+html`, `frameworkVersion` `fastlectures-visual-explorer/1`, and `refreshSeconds` `0`.
...
| CA-07 | Enter `Focus the view on the Widget you just created.` Then enter `Revert FastLectures Agent's latest content change.` | Set-view changes only the view and does not dirty content history. Revert accepts only the latest Agent change and cannot revert an older change or a user's manual edit. |
| DL-01 | Select a Visual Explorer or another Widget created by FastLectures Agent and click **Download Widget image** in the object's side action bar. | Preparing and success/failure feedback appear. A clean PNG of the current Widget is exported without Canvas chrome, selection outlines, or Agent activity. The filename is safe and the Widget state is unchanged. |
