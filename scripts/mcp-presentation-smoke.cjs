...
const directory=fs.mkdtempSync(path.join(os.tmpdir(),"fastlectures-presentation-"));
app.setPath("userData",path.join(directory,"browser"));
Object.assign(process.env,{NODE_ENV:"test",FASTLECTURES_TEST_OPEN_ACCESS:"1",FASTLECTURES_STATE_DIR:path.join(directory,"state"),HOST:"127.0.0.1",PORT:"0",AI_PROVIDER:"api",AI_API_KEY:"test-only",AI_API_URL:"http://127.0.0.1:1/v1",AI_API_MODEL:"test",FASTLECTURES_CANVAS_AGENT_AUTO_OPEN:"false",FASTLECTURES_REQUEST_TRACE:"false"});
...
app.whenReady().then(async()=>{
 try{
  ...
  let listed;await waitFor(async()=>{listed=await call("fastlectures_list_canvases",{});return listed.canvases.length===1;},"discovery");
  const target=listed.canvases[0],session=await call("fastlectures_start_session",{instanceId:target.instanceId,canvasId:target.canvasId,title:"展示与反馈验收",client:"Test",sessionKey:"presentation-smoke"});
  ...
  const html=title=>`<!doctype html><html lang="zh"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}body{margin:0;padding:24px;background:#fff;color:#202124;font:16px/1.6 system-ui}h1{font-size:26px;line-height:1.3;margin:0 0 16px}button{font:inherit;min-height:36px;line-height:1.8;padding:4px 12px;margin-top:16px}p{overflow-wrap:anywhere}</style><h1>${title}</h1><p>查看方案，直接标注需要调整的地方。选择后，外部对话读取指令并继续修改。</p><button data-fastlectures-action="choose" data-fastlectures-prompt="保留当前布局，并简化说明文字">采用这个方案并继续修改</button></html>`;
  const main=await call("fastlectures_present_widget",{sessionId:session.sessionId,artifactId:"main",title:"主要方案",html:html("主要方案"),presentation:{intent:"review",size:"base"}});
  await pause(1300);
  const compare=await call("fastlectures_present_widget",{sessionId:session.sessionId,artifactId:"alternative",title:"备选方案",html:html("备选方案"),presentation:{intent:"compare",role:"alternative",relativeTo:"main",relation:"beside"}});
  const inspected=await call("fastlectures_inspect_session",{sessionId:session.sessionId});
  ...
  const geometry=async objectId=>{const file=await call("fastlectures_read_file",{sessionId:session.sessionId,path:`objects/${objectId}/geometry.json`});return JSON.parse(file.content);};
  ...
  const before=await call("fastlectures_read_file",{sessionId:session.sessionId,path:"canvas.json"});
  for(const [name,width,height] of [["mobile",390,844],["desktop",1200,800]]){
   const capture=await call("fastlectures_present_widget",{sessionId:session.sessionId,artifactId:`inspect-${name}`,title:`检查 ${name}`,html:html(`检查 ${name}`),width,height,presentation:{intent:"inspect"},capture:true});
  ...
  const after=await call("fastlectures_read_file",{sessionId:session.sessionId,path:"canvas.json"});assert.equal(before.contentHash,after.contentHash);
  const source=await call("fastlectures_present_widget",{sessionId:session.sessionId,artifactId:"main",title:"主要方案",html:html("修订后的方案"),width:390});assert.equal(source.objectId,main.objectId);assert.deepEqual(await geometry(main.objectId),a);
  ...
  for(const [name,width,zoom] of [["narrow",700,1],["zoom",1440,2]]){
   ...
   const next=await call("fastlectures_present_widget",{sessionId:session.sessionId,artifactId:name,title:"继续对比",html:html("继续对比"),presentation:{intent:"compare",relativeTo:"main",relation:"beside",attention:"quiet"}});
