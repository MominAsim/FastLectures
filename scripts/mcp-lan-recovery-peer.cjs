...
async function main(){
 ...
 async function execute(method,params={}){
  if(method==='open'){
   ...
   directory=fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()),'fastlectures-recovery-peer-'));const host=path.join(directory,'hosts',trust.hostId);fs.mkdirSync(host,{recursive:true,mode:0o700});fs.writeFileSync(path.join(host,'credentials.json'),JSON.stringify(trust),{mode:0o600});fs.writeFileSync(path.join(host,'ca.pem'),trust.certificatePem,{mode:0o600});cache=path.join(host,'endpoint.json');
