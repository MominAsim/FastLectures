...
const ROOT=path.resolve(__dirname,".."),directory=fs.mkdtempSync(path.join(os.tmpdir(),"fastlectures-mcp-browser-"));
...
Object.assign(process.env,{NODE_ENV:"test",FASTLECTURES_TEST_OPEN_ACCESS:"1",FASTLECTURES_STATE_DIR:path.join(directory,"state"),HOST:"127.0.0.1",PORT:"0",AI_PROVIDER:"api",AI_API_KEY:"mcp-test-key",AI_API_URL:"http://127.0.0.1:1/v1",AI_API_MODEL:"mcp-test",FASTLECTURES_CANVAS_AGENT_AUTO_OPEN:"false",FASTLECTURES_REQUEST_TRACE:"false"});
...
app.whenReady().then(async()=>{
  try{
    ...
    const record=readRecords(recordsDirectory(process.env.FASTLECTURES_STATE_DIR))[0],ownerId=crypto.randomUUID();
    ...
    let listed;await waitFor(async()=>{listed=await call("fastlectures_list_canvases",{});return listed.canvases.length===1;},"canvas opt in");
    ...
    const target=listed.canvases[0],one=await call("fastlectures_start_session",{instanceId:target.instanceId,canvasId:target.canvasId,title:"Checkout implementation",client:"Codex",sessionKey:"smoke-one"});
    ...
    window.setSize(1440,1000);window.webContents.setZoomFactor(1);
    const two=await call("fastlectures_start_session",{instanceId:target.instanceId,canvasId:target.canvasId,title:"API regression review",client:"Kimi",sessionKey:"smoke-two"});
    await call("fastlectures_update_session",{sessionId:one.sessionId,summary:"Implement the checkout layout and verify narrow screens.",steps:[{id:"plan",label:"Confirm interaction and API contract",status:"done"},{id:"build",label:"Build the checkout preview",status:"working"},{id:"test",label:"Verify layout and keyboard behavior",status:"pending"}],events:[{id:"e1",text:"The form keeps one primary action.",kind:"evidence"}]});
    await call("fastlectures_update_session",{sessionId:two.sessionId,summary:"Checking error responses and transaction boundaries.",steps:[{id:"review",label:"Review API contract",status:"working"}]});
    await pause(250);
    ...
    const preview=await call("fastlectures_present_widget",{sessionId:one.sessionId,artifactId:"checkout",title:"Checkout preview",html,width:960,height:640});
    const capture=await call("fastlectures_capture_widget",{sessionId:one.sessionId,artifactId:"checkout"});
    assert.ok(capture.image.bytes>100);fs.writeFileSync(path.join(directory,"widget.webp"),Buffer.from(capture.image.data,"base64"));
    const next=await call("fastlectures_present_widget",{sessionId:one.sessionId,artifactId:"checkout",title:"Checkout preview",html:html.replace("Complete your order","Review your order"),width:390,capture:true});assert.equal(next.objectId,preview.objectId);
    ...
    const inFlight=call("fastlectures_present_widget",{sessionId:one.sessionId,artifactId:"checkout",title:"Checkout preview",html:html.replace("Complete your order","Review your order"),width:390});
    ...
    for(const id of ['layout-a','layout-b','layout-c'])await call("fastlectures_present_widget",{sessionId:one.sessionId,artifactId:id,title:id,html:`<html><body style="font:18px system-ui;padding:24px;background:#f6f9f8"><h2>${id}</h2><p>FastLectures places this preview within its task.</p></body></html>`,width:800,height:480});
    await pause(1200);
    const arranged=(await call("fastlectures_inspect_session",{sessionId:one.sessionId})).browser.artifacts.filter(item=>item.artifactId.startsWith('layout-'));
    ...
    await call("fastlectures_present_widget",{sessionId:one.sessionId,artifactId:'layout-d',title:'Later preview',html:'<p>A new preview while the user is navigating.</p>',width:800,height:480});await pause(1200);
    ...
    const native=await call('fastlectures_draw',{...drawingArgs,capture:true});assert.equal(native.objectIds.length,5);assert.ok(native.image.bytes<=700*1024);
    fs.writeFileSync(path.join(directory,'mcp-native-drawing.webp'),Buffer.from(native.image.data,'base64'));
    const nativeUpdate=await call('fastlectures_draw',{...drawingArgs,items:drawingArgs.items.map(item=>item.id==='plan'?{...item,text:'Plan complete'}:item)});assert.deepEqual(nativeUpdate.objectIds,native.objectIds);
    const plotted=await call('fastlectures_plot',{sessionId:one.sessionId,artifactId:'native-function',title:'Sine curve',expression:'sin(x)',xMin:-6.28,xMax:6.28,width:800,height:500,capture:true});
    ...
    const undone=await call('fastlectures_inspect_session',{sessionId:one.sessionId});assert.equal(undone.browser.artifacts.find(item=>item.artifactId==='native-function').removed,true,'plot participates in Canvas undo');
    await js("document.querySelector('[data-action=redo]').click()");
    const redone=await call('fastlectures_inspect_session',{sessionId:one.sessionId});assert.equal(redone.browser.artifacts.find(item=>item.artifactId==='native-function').removed,undefined,'plot participates in Canvas redo');
    await assert.rejects(call('fastlectures_plot',{sessionId:one.sessionId,artifactId:'bad-function',title:'Invalid',expression:'globalThis.alert(1)'}));
    assert.equal(await js("document.querySelectorAll('.canvas-widget').length"),frameCount,'native drawing creates no iframe');
    const nativeInspect=await call('fastlectures_inspect_session',{sessionId:one.sessionId});
    ...
    await js("document.querySelector('#settingsBtn').click();document.querySelector('#settingsNavMcp').click()");
    const baseline=await call("fastlectures_read_feedback",{sessionId:one.sessionId});assert.equal(baseline.changeCount,0,"AI output is not user feedback");
    ...
    let feedback;await waitFor(async()=>{feedback=await call("fastlectures_read_feedback",{sessionId:one.sessionId});return feedback.hasFeedback;},"committed text feedback");
    ...
    const inkFeedback=await call("fastlectures_read_feedback",{sessionId:one.sessionId,after:typedCursor,capture:true});assert.equal(inkFeedback.hasFeedback,true);assert.ok(inkFeedback.image.bytes>0);
    ...
    await waitFor(async()=>{feedback=await call("fastlectures_read_feedback",{sessionId:one.sessionId,after:inkFeedback.nextCursor});return feedback.hasFeedback;},"imported image feedback");
    const imageFeedback=await call("fastlectures_read_feedback",{sessionId:one.sessionId,after:inkFeedback.nextCursor,capture:true});assert.ok(imageFeedback.image.bytes>0);assert.equal(imageFeedback.pixelVerified,true);
    fs.writeFileSync(path.join(directory,'user-feedback-image.webp'),Buffer.from(imageFeedback.image.data,'base64'));
    assert.equal((await call("fastlectures_read_feedback",{sessionId:one.sessionId,after:imageFeedback.nextCursor})).changeCount,0);
    const peerFeedback=await call("fastlectures_read_feedback",{sessionId:two.sessionId});assert.ok(peerFeedback.hasFeedback,"one reader must not consume another session's feedback");
    ...
    const probe=await call("fastlectures_read_feedback",{sessionId:one.sessionId,capture:false});assert.equal(probe.image,undefined);assert.equal(probe.hasFeedback,true);
    report.checks.push({name:"user-text-stroke-image-feedback-and-cursors",ok:true});
    await js("document.querySelector('#settingsBtn').click();document.querySelector('#settingsNavMcp').click()");
    const inspected=await call("fastlectures_inspect_session",{sessionId:one.sessionId});assert.equal(inspected.browser.artifacts.length,7);
    ...
    await js("document.querySelector('#settingsBtn').click();document.querySelector('#settingsNavMcp').click()");
    await call("fastlectures_update_session",{sessionId:one.sessionId,status:"done",summary:"Preview updated and captured successfully."});await call("fastlectures_close_session",{sessionId:one.sessionId});
    await js("document.querySelector('#mcpEnabled').click()");
    await waitFor(async()=>!(await call("fastlectures_list_canvases",{})).canvases.length,"revocation");
