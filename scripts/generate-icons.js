...
<<<<<<< HEAD
  source = path.join(ROOT, "public", "fastlectures-mark.png"),
  wordmarkSource = path.join(ROOT, "public", "fastlectures-readme-header.png");
=======
...
<<<<<<< HEAD
    const output = path.join(generated, `fastlectures-${size}.png`);
    await png(size, output);
    files.set(size, output);
  }
  fs.copyFileSync(files.get(512), path.join(iconRoot, "fastlectures.png"));
  fs.copyFileSync(files.get(1024), path.join(iconRoot, "fastlectures-1024.png"));
  await installerGif(path.join(iconRoot, "fastlectures-install.gif"));
...
>>>>>>> 97ac987080073424039f82c866723e028d0bc78b
  ...
  fs.writeFileSync(path.join(iconRoot, "fastlectures.icns"), icns);
  fs.writeFileSync(path.join(iconRoot, "fastlectures.ico"), ico);
  console.log(`Generated FastLectures desktop icons in ${iconRoot}`);
}
