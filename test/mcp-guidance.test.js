...
const { FastLecturesStdioServer, INSTRUCTIONS } = require("../src/server/mcp/stdio.js");
...
test("shared guidance preserves the complete authoritative design and scientific documents", () => {
  ...
  const body = contract.slice(contract.indexOf("Do not start from visual decoration."), contract.indexOf("## FastLectures Agent source and invocation"));
...
test("default and explicit brief deliver the complete Visual Explorer contract through Agent and MCP", async () => {
  ...
  const agent = createDocumentTools({id:"guidance-quality-test"}).find(tool => tool.name === "fastlectures_get_guidance");
  const messages = [];
  const server = new FastLecturesStdioServer({output:{write:text => messages.push(JSON.parse(text))}});
  ...
  for (const args of [{id:"visual-explorer"}, {id:"visual-explorer",detail:"brief"}]) {
    ...
    await server.handle({jsonrpc:"2.0",id:messages.length+1,method:"tools/call",params:{name:"fastlectures_get_guidance",arguments:args}});
...
test("routing keeps page UI separate and default guidance compact", () => {
  ...
  assert.match(getAuthoringGuidance("visual-explorer","full").document, /Bare function graphs use fastlectures_plot/);
  assert.match(INSTRUCTIONS, /fastlectures_get_guidance/);
...
test("guidance is discoverable and validates strict arguments without changing old tool contracts", () => {
  const tool = TOOLS.find(tool => tool.name === "fastlectures_get_guidance");
  ...
  assert.deepEqual(validateToolArguments("fastlectures_list_canvases", {}), {});
});
...
test("stdio returns guidance and legacy prompt without discovery or a live instance", async () => {
  ...
  assert.ok(listed.result.tools.some(tool => tool.name === "fastlectures_get_guidance"));
  for (const id of GUIDANCE_IDS) {
    const response = await call("tools/call", {name:"fastlectures_get_guidance",arguments:{id,detail:"full"}});
    assert.deepEqual(response.result.structuredContent, getAuthoringGuidance(id,"full"));
  }
  const invalid = await call("tools/call", {name:"fastlectures_get_guidance",arguments:{id:"private"}});
  assert.equal(invalid.result.isError, true);
  assert.equal(invalid.result.structuredContent.code, "invalid_arguments");
  const prompt = await call("prompts/get", {name:"fastlectures_visual_explorer",arguments:{}});
...
test("HTML guidance establishes responsive viewport sizing without a planning round trip", () => {
  ...
  const tool = TOOLS.find(tool => tool.name === "fastlectures_present_widget");
...
test("rendering guidance separates Canvas delivery, chat source, and explicit implementation bans", async () => {
  ...
  assert.match(TOOLS.find(tool => tool.name === "fastlectures_present_widget").description, /including static diagrams/);
  assert.match(TOOLS.find(tool => tool.name === "fastlectures_draw").description, /few simple native.*explicitly requested native/);
});
...
test("Agent and MCP use the canonical 1.2.0 Visual Explorer selection conditions", async () => {
  ...
  assert.equal(VISUAL_EXPLORER_SELECTION, paragraph.replace('host-native `canvas_create` `type:"plot"`', '`fastlectures_plot`'));
