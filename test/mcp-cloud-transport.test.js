...
<<<<<<< HEAD
function setup(runtime='cloud',fetch){Socket.instances=[];const window=new EventTarget(),timers=new Map();window.FASTLECTURES_CONFIG={runtime};let sequence=0;vm.runInNewContext(fs.readFileSync(require.resolve('../public/mcp-cloud-transport.js'),'utf8'),{window,fetch,sessionStorage:{getItem:()=>null},location:{protocol:'https:',host:'fastlectures.test'},WebSocket:Socket,EventTarget,Event,MessageEvent,setTimeout:(fn,ms)=>{timers.set(++sequence,{fn,ms});return sequence;},clearTimeout:id=>timers.delete(id)});return {window,timers};}
=======
...
test('one opted-in Canvas routes direct cloud and device calls separately, including colliding request IDs',()=>{
  const {window}=setup();window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'8c8d3342-8258-470e-8ca1-61b732222024'};
  const facade=new window.FastLecturesCloudMcpSocket(),cloud=Socket.instances[0],received=[];facade.addEventListener('message',event=>received.push(JSON.parse(event.data)));
...
test('a device disconnect cancels only its work; retries are bounded and opt-out stops them',()=>{
  const {window,timers}=setup();window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'8c8d3342-8258-470e-8ca1-61b732222024'};
  const facade=new window.FastLecturesCloudMcpSocket(),received=[];facade.addEventListener('message',event=>received.push(JSON.parse(event.data)));Socket.instances[0].open();const device=Socket.instances[1];device.open();
...
test('cloud works without a linked device and a later exact-device change replaces only the device channel',()=>{
  const {window,timers}=setup(),facade=new window.FastLecturesCloudMcpSocket();Socket.instances[0].open();assert.equal(Socket.instances.length,1);
  window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'8c8d3342-8258-470e-8ca1-61b732222024'};window.dispatchEvent(new Event('fastlectures:remote-cloud-status'));Socket.instances[1].open();
  window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'ab51c889-54fd-47ef-b815-15335baaa4e5'};window.dispatchEvent(new Event('fastlectures:remote-cloud-status'));assert.equal(Socket.instances[1].readyState,3);
...
test('anonymous local MCP works offline; signing in attaches cloud through the host with no device token in browser URLs',async()=>{
  let enabled=false;const {window}=setup('local',async()=>({ok:true,json:async()=>({cloudMcpEnabled:enabled})})),facade=new window.FastLecturesCloudMcpSocket();
...
test('transient status failure retries with bounded backoff; a setting changed during lookup is not lost',async()=>{
  ...
  const facade=new window.FastLecturesCloudMcpSocket();Socket.instances[0].open();await new Promise(setImmediate);assert.equal(timers.size,1);
  fail=false;const pending=facade.connectDevice();window.dispatchEvent(new Event('fastlectures:cloud-account-changed'));
...
test('availability waits for each authenticated ready frame and drops only the lost channel',()=>{
  const {window}=setup();window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'8c8d3342-8258-470e-8ca1-61b732222024',deviceOnline:true};
  const facade=new window.FastLecturesCloudMcpSocket();let changes=0;facade.addEventListener('availabilitychange',()=>changes++);
...
test('local primary is Local; the device channel becomes Cloud only after ready',async()=>{
  ...
  let status;window.FastLecturesMcpSettings={setDeviceStatus:value=>{status=value;}};
  const facade=new window.FastLecturesCloudMcpSocket(),local=Socket.instances[0];local.open();await new Promise(setImmediate);
...
test('offline linked device cannot appear online from a stale device ID',()=>{
  const {window}=setup();window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceId:'8c8d3342-8258-470e-8ca1-61b732222024',deviceOnline:false};
  const facade=new window.FastLecturesCloudMcpSocket();Socket.instances[0].open();Socket.instances[0].receive({type:'ready'});
