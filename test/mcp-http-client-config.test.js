...
test('Codex preserves array tables before and after the replaced FastLectures subtree',()=>{
  ...
  const output=replaceCodex(before+'[mcp_servers.fastlectures]\ncommand = "old"\n[mcp_servers.fastlectures.env]\nOLD = "remove"\n'+after,configurationFragment('codex',connection));
  ...
  assert.equal((output.match(/\[mcp_servers.fastlectures\]/g)||[]).length,1);
});
test('Codex distinguishes quoted dotted keys and supports quoted bracket characters',()=>{
  const {replaceCodex}=require('../src/server/mcp/http-client-config');
  const kept='["mcp_servers.fastlectures"]\nkeep = 1\n[mcp_servers."fastlectures.env"]\nkeep = 2\n[projects."folder]name"]\nkeep = 3\n';
  const output=replaceCodex(kept+'["mcp_servers" . "fastlectures"]\ncommand = "remove"\n',configurationFragment('codex',connection));
...
test('Codex rejects malformed or ambiguous headers with a stable error code',()=>{
  const {replaceCodex}=require('../src/server/mcp/http-client-config');
  for(const source of ['[[other]\nx=1','[other.]\nx=1','[[mcp_servers.fastlectures]]\nx=1'])assert.throws(()=>replaceCodex(source,''),{code:'MCP_CONFIG_UNSUPPORTED'});
});
function home(t){const dir=fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()),'fastlectures-http-'));t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));return dir;}
test('Claude preserves other entries and project scope',t=>{const dir=home(t);fs.writeFileSync(dir+'/.claude.json',JSON.stringify({projects:{demo:{mcpServers:{x:{}}}},mcpServers:{other:{url:'else'}}}));const result=configureHttpClient('claude',connection,{home:dir,env:{}});const config=JSON.parse(fs.readFileSync(dir+'/.claude.json'));assert.ok(config.projects.demo);assert.equal(config.mcpServers.other.url,'else');assert.equal(config.mcpServers.fastlectures.type,'http');assert.equal(result.processEnvironment.NODE_EXTRA_CA_CERTS,connection.caFile);assert.ok(result.trustRequired&&result.reloadRequired);assert.ok(!JSON.stringify(result).includes(connection.accessToken));});
test('Codex removes only FastLectures tables and preserves other subtable scopes',t=>{const dir=home(t);fs.mkdirSync(dir+'/.codex');fs.writeFileSync(dir+'/.codex/config.toml','model = "x"\n[mcp_servers.fastlectures]\ncommand = "old"\n[mcp_servers.fastlectures.env]\nX = "old"\n[mcp_servers.other]\nurl = "other"\n[mcp_servers.other.http_headers]\nX = "keep"\n');configureHttpClient('codex',connection,{home:dir,env:{}});const result=fs.readFileSync(dir+'/.codex/config.toml','utf8');assert.ok(result.includes('X = "keep"'));assert.ok(!result.includes('"old"'));assert.equal((result.match(/\[mcp_servers.fastlectures\]/g)||[]).length,1);});
...
test('HTTP to stdio migration removes obsolete connection fields and identical writes are no-ops',t=>{
 ...
 fs.mkdirSync(dir+'/.codex');const codexFile=dir+'/.codex/config.toml';fs.writeFileSync(codexFile,'model = "keep-model"\n[mcp_servers.fastlectures]\nurl = "https://old.local/mcp"\nbearer_token_env_var = "OLD_TOKEN"\n[mcp_servers.fastlectures.http_headers]\nAuthorization = "secret-old"\n[mcp_servers.fastlectures.env]\nHTTP_PROXY = "obsolete"\n[mcp_servers.other]\ncommand = "other-command"\n');
 const claudeFile=dir+'/.claude.json';fs.writeFileSync(claudeFile,JSON.stringify({unrelated:true,mcpServers:{other:{command:'other-command'},fastlectures:{type:'http',url:'https://old.local/mcp',headers:{Authorization:'secret-old'},env:{HTTP_PROXY:'obsolete'}}}}));
