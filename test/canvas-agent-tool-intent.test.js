...
test("FastLectures Agent tool hints name visual capabilities in one sentence",()=>{
...
test("FastLectures Agent search hints include compact search keywords in the same sentence",()=>{
  assert.equal(intent("deepseek_search",{query:"WebGPU browser support 2026"}),"Search the web · “WebGPU browser support 2026”");
  assert.equal(intent("duckduckgo_search",{query:"  FastLectures\nAgent  "}),"Search the web · “FastLectures Agent”");
...
test("FastLectures Agent gives every current host tool a specific one-sentence hint",()=>{
  const calls={
    ...
    tavily_search:{query:"FastLectures"},deepseek_search:{query:"FastLectures"},research_search:{query:"FastLectures"},github_repository_search:{query:"FastLectures"},duckduckgo_search:{query:"FastLectures"},web_read:{url:"https://example.com"},stock_symbol_search:{query:"Apple"},stock_market_data:{symbol:"AAPL"},load_visual_skill:{skill:"physics-2d"},load_widget_contract:{route:"professional-diagrams"},
  };
