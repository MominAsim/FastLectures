...
const read=file=>fs.readFileSync(path.join(__dirname,"..",file),"utf8"),
  ...
  forward=host.slice(host.indexOf('if (message.type === "fastlectures-widget-updated") {'),host.indexOf('    } else if (message.type === "fastlectures-widget-snapshot"'))+'}';
function harness(){
...
test("author updates invalidate snapshots but only loaded updated releases the MCP document wait",async()=>{
  ...
  try {
    await h.message({type:"fastlectures-widget-updated"});
    ...
    await h.message({type:"fastlectures-widget-capture-ready"});assert.equal(completed,false);assert.equal(h.widget.mcpDocumentLoaded,false);
    await h.message({type:"fastlectures-widget-updated",loaded:true});await pending;
...
test("host preserves current-runtime load notification and rejects stale-runtime load notification",()=>{
  const h=harness();h.forward({type:"fastlectures-widget-updated"});
  assert.equal(h.forwarded.length,1);assert.equal(h.forwarded[0].loaded,false);
  h.forward({type:"fastlectures-widget-updated",loaded:true,runtimeVersion:3});assert.equal(h.forwarded.length,1);
  h.forward({type:"fastlectures-widget-updated",loaded:true,runtimeVersion:4});
  ...
  h.forward({type:"fastlectures-widget-updated"});assert.equal(h.forwarded.length,2);
});
test("runtime window load announces loaded once with the current runtime version",()=>{
  ...
  vm.runInNewContext(notify+registration,{UPDATED:"fastlectures-widget-updated",runtimeVersion:4,parent:{postMessage:message=>messages.push(message)},addEventListener:events.addEventListener.bind(events)});
...
test("ordinary content updates still reject an in-flight stale snapshot",async()=>{
  ...
  await h.message({type:"fastlectures-widget-updated"});
  await h.message({type:"fastlectures-widget-snapshot",requestId:"capture",dataUrl:"data:image/png;base64,AQ==",width:100,height:50});
