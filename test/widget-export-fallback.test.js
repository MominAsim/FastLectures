...
test('image export reuses available pixels after capture failure, but never silently drops a missing Widget',async()=>{
 for(const mode of ["available","missing","stale"]){
  ...
  const env={exportRegion:()=>({x:0,y:0,w:400,h:400}),prepareVisibleWidgetSnapshots:async()=>{throw error;},capturableWidgets:()=>[{snapshotImage:mode==="missing"?null:{},snapshotVersion:mode==="stale"?0:1,contentVersion:1}],debug(){},CANVAS_DOWNLOAD_RESOLUTION_SCALE:1,EXPORT_MAX_DIMENSION:2048,EXPORT_MAX_PIXELS:4194304,offscreen:()=>canvas,performance,state:{paint:{paper:'#fff'}},drawAnimationsToContext(){},drawWidgetsToContext(){drawn=true;},drawImagesToContext(){},drawTextBoxesToContext(){},tiles:new Map(),drawSharpOverlays(){}};
