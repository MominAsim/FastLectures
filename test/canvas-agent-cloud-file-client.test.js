...
function fixture(request){
 const context=vm.createContext({location:{pathname:'/canvas/11111111-1111-4111-8111-111111111111'},window:{FASTLECTURES_CONFIG:{hostedDocumentFiles:true}},canvasAgent:{currentConversation:{id:'conversation-test'}},canvasAgentCloudSavedCanvasId:()=>"saved",canvasAgentCloudCanvasId:()=>"11111111-1111-4111-8111-111111111111",t:key=>key,canvasAgentUsesCloudHost:()=>true,canvasAgentProjectRequest:request,encodeURIComponent});
...
test('Cloud picker uploads raw bytes bound to logical conversation with strict format/size gate',async()=>{
 ...
 assert.equal(calls.length,1);assert.match(calls[0].url,/files\/conversation-test$/);assert.equal(calls[0].options.body,file);assert.equal(calls[0].options.headers['content-type'],'application/x-fastlectures-document');assert.equal(result.cloudScope.conversationId,'conversation-test');
});
