...
test('HTTPS bridge forwards, recovers expired owner and releases on idle and EOF',async t=>{
 const directory=fs.mkdtempSync(path.join(os.tmpdir(),'fastlectures-session-'));let clock=0;const disposed=[];
 ...
 clock=11;send(io,3,'tools/call',{name:'fastlectures_list_canvases',arguments:{}});await until(()=>io.messages.length===3);assert.ok(io.messages[2].result.structuredContent.ownerId);assert.notEqual(client.sessionId,original);
...
test('Canvas session recovery preserves document and maps original session ID',async t=>{
 ...
 if(message?.method==='tools/call'){calls.push(message.params);return {status:200,body:{jsonrpc:'2.0',id:message.id,result:{structuredContent:message.params.name==='fastlectures_start_session'?{sessionId:`canvas-${owner}`,documentId:'doc-1'}:{sessionId:message.params.arguments.sessionId}}}};}
 return {status:202};}});t.after(()=>client.close());
 send(io,1,'initialize',{});send(io,2,'tools/call',{name:'fastlectures_start_session',arguments:{client:'codex',sessionKey:'stable',target:'current'}});await until(()=>io.messages.length===2);expired=true;
 send(io,3,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:'canvas-1'}});await until(()=>io.messages.length===3);assert.equal(io.messages[2].result.structuredContent.sessionId,'canvas-1');assert.equal(calls[1].arguments.sessionKey,'stable');assert.equal(calls[1].arguments.documentId,'doc-1');assert.equal(calls[1].arguments.target,undefined);
 expired=true;send(io,4,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:'canvas-1'}});await until(()=>io.messages.length===4);assert.equal(io.messages[3].result.structuredContent.sessionId,'canvas-1');assert.equal(calls.at(-1).arguments.sessionId,'canvas-3');assert.equal(client.rememberedCount,1);assert.equal(resolutions,3);
});
...
test('remembered handles are capped before dispatch and successful close frees capacity',async t=>{
 ...
 if(message?.params?.name==='fastlectures_start_session')return {status:200,body:{jsonrpc:'2.0',id:message.id,result:{structuredContent:{sessionId:`s${++starts}`,documentId:'doc'}}}};
 ...
 for(let n=0;n<128;n++){send(io,n+2,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:`k${n}`}});await until(()=>io.messages.length===n+2);}
 send(io,130,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:'overflow'}});await until(()=>io.messages.length===130);assert.match(io.messages.at(-1).error.message,/128/);assert.equal(starts,128);assert.equal(client.rememberedCount,128);
 send(io,131,'tools/call',{name:'fastlectures_close_session',arguments:{sessionId:'s1'}});await until(()=>io.messages.length===131);assert.equal(client.rememberedCount,127);
});
...
test('same live CLI reconnects through updated cache then discovery after idle HTTP release, preserving document',async t=>{
 const discovery=require('../src/server/mcp/discovery-client.js');const directory=fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()),'fastlectures-idle-reconnect-'));let clock=0,current,discoveries=0;const attempts=[];
 const service=createDirectHttpService({stateDirectory:directory,preferredPort:0,getHostnames:()=>[],getAddresses:()=>[],announce:()=>({close(){}}),disposeOwner:()=>{},callTool:async(owner,name,args)=>name==='fastlectures_start_session'?{sessionId:`business-${owner}`,documentId:'persistent-doc'}:{sessionId:args.sessionId,documentId:'persistent-doc'}});
 ...
 t.after(async()=>{await client.close();await service.close();fs.rmSync(directory,{recursive:true,force:true});});
 send(io,1,'initialize',{});send(io,2,'tools/call',{name:'fastlectures_start_session',arguments:{client:'test',sessionKey:'persistent-key'}});await until(()=>io.messages.length===2);const original=io.messages[1].result.structuredContent.sessionId;
 ...
 send(io,3,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:original}});await until(()=>io.messages.length===3);assert.equal(io.messages[2].result.structuredContent.documentId,'persistent-doc');assert.equal(io.messages[2].result.structuredContent.sessionId,original);assert.deepEqual(attempts,[first,second]);assert.equal(discoveries,0);
...
test('explicit business session expiry restores stable document before retrying the rejected operation',async t=>{
 ...
 if(message.params.name==='fastlectures_start_session'){starts++;if(starts===2){assert.equal(message.params.arguments.documentId,'doc');assert.equal(message.params.arguments.sessionKey,'stable');}result={structuredContent:{sessionId:`handle-${starts}`,documentId:'doc'}};}
 else if(!rejections++){result={isError:true,structuredContent:{code:'session_expired',details:{retry:'fastlectures_start_session',sessionKey:'stable',documentId:'doc'}}};}
 else{mutations++;assert.equal(message.params.arguments.sessionId,'handle-2');result={structuredContent:{sessionId:'handle-2'}};}
 return {status:200,body:{jsonrpc:'2.0',id:message.id,result}};}});t.after(()=>client.close());
 send(io,1,'initialize',{});send(io,2,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:'stable'}});send(io,3,'tools/call',{name:'mutation',arguments:{sessionId:'handle-1'}});await until(()=>io.messages.length===3);assert.equal(starts,2);assert.equal(mutations,1);assert.equal(io.messages[2].result.structuredContent.sessionId,'handle-1');
});
...
test('document mismatch during explicit expiry recovery never replays the original mutation',async t=>{
 ...
 const result=message.params.name==='fastlectures_start_session'?{structuredContent:{sessionId:`handle-${++starts}`,documentId:starts===1?'original':'wrong-document'}}:(operations++,{isError:true,structuredContent:{code:'session_expired',details:{retry:'fastlectures_start_session',documentId:'original',sessionKey:'stable'}}});return {status:200,body:{jsonrpc:'2.0',id:message.id,result}};}});t.after(()=>client.close());send(io,1,'initialize',{});send(io,2,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:'stable'}});send(io,3,'tools/call',{name:'mutation',arguments:{sessionId:'handle-1'}});await until(()=>io.messages.length===3);assert.match(io.messages[2].error.message,/original document/);assert.equal(operations,1);assert.equal(starts,2);
});
...
test('HTTPS pool reuses authenticated TLS connections, isolates credentials and honors cancellation',async t=>{
 ...
 const directory=fs.mkdtempSync(path.join(os.tmpdir(),'fastlectures-pool-')),identity=loadDirectHttpIdentity(directory),pool=createTransportPool();let connections=0,slowMode=false;
...
test('bounded scheduler runs independent documents and directory queries while preserving handle order',async t=>{
 ...
 return {status:200,body:{jsonrpc:'2.0',id:message?.id,result:{}}};}});t.after(()=>client.close());
 send(io,1,'initialize',{});await until(()=>io.messages.length===1);
 send(io,2,'tools/call',{name:'fastlectures_capture_canvas',arguments:{sessionId:'a'}});send(io,3,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:'a'}});
 send(io,4,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:'b'}});send(io,5,'tools/call',{name:'fastlectures_list_canvases',arguments:{}});
 ...
 for(let id=6;id<12;id++)send(io,id,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:`doc-${id}`}});
...
test('aliases of the same document stay ordered during handle restoration after owner expiry',async t=>{
 ...
 if(message?.params?.name==='fastlectures_start_session'){
 ...
 return {status:200,body:{jsonrpc:'2.0',id:message?.id,result:{}}};}});t.after(()=>client.close());
 send(io,1,'initialize',{});send(io,2,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:'alias-a'}});send(io,3,'tools/call',{name:'fastlectures_start_session',arguments:{sessionKey:'alias-b'}});await until(()=>client.pendingCount===0);
 expire=true;send(io,4,'ping');await until(()=>client.pendingCount===0);assert.equal(owner,2);
 send(io,5,'tools/call',{name:'fastlectures_capture_canvas',arguments:{sessionId:'alias-a-1'}});send(io,6,'tools/call',{name:'fastlectures_get_canvas',arguments:{sessionId:'alias-b-1'}});
...
test('cancellation immediately after dequeue prevents dispatch across the ensure await',async t=>{
 ...
 send(io,1,'initialize',{});await until(()=>io.messages.length===1);
 io.stdin.write(JSON.stringify({jsonrpc:'2.0',id:2,method:'tools/call',params:{name:'fastlectures_patch_file',arguments:{sessionId:'a'}}})+'\n'+JSON.stringify({jsonrpc:'2.0',method:'notifications/cancelled',params:{requestId:2}})+'\n');
