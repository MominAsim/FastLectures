...
function runPageScale(config, storedScale = null) {
  ...
  const browserWindow = { FASTLECTURES_CONFIG:config, fastlecturesDesktop:{ setPageScale:value => desktopCalls.push(value) } };
...
test("Canvas page scale exposes four validated choices with mutually exclusive desktop/web paths", () => {
  ...
  assert.deepEqual(web.classes, ["fastlectures-web-page-scale"]);
  assert.equal(web.dataset.fastlecturesPageScale, "125");
  assert.equal(web.browserWindow.FastLecturesPageScale.apply(0.9), 0.9);
  assert.equal(web.storage.get(CANVAS_PAGE_SCALE_STORAGE_KEY), "0.9");
  assert.equal(web.dataset.fastlecturesPageScale, "90");
  assert.equal(web.browserWindow.FastLecturesPageScale.apply(1.25), 1.25);
  assert.equal(web.storage.get(CANVAS_PAGE_SCALE_STORAGE_KEY), "1.25");
  assert.equal(web.dataset.fastlecturesPageScale, "125");
  assert.equal(web.browserWindow.FastLecturesPageScale.apply(1), 1);
  assert.deepEqual(web.classes, []);
  assert.equal(web.dataset.fastlecturesPageScale, undefined);
  assert.equal(web.browserWindow.FastLecturesPageScale.apply(0.9), 0.9);
  assert.deepEqual(web.classes, ["fastlectures-web-page-scale"]);
  assert.equal(web.dataset.fastlecturesPageScale, "90");
  ...
  assert.match(main, /ipcMain\.handle\("fastlectures:set-page-scale"[\s\S]*?fromCanvas\(event\)[\s\S]*?setZoomFactor\(scale\)/);
  assert.match(preload, /setPageScale:scale => ipcRenderer\.invoke\("fastlectures:set-page-scale", scale\)/);
  assert.match(css, /@media \(min-width: 701px\) \{\s*html\.fastlectures-web-page-scale\[data-fastlectures-page-scale="90"\][\s\S]*?--fastlectures-canvas-page-scale:\s*\.9[\s\S]*?--fastlectures-canvas-page-viewport-width:\s*111\.11111111111111vw/);
  assert.match(css, /html\.fastlectures-web-page-scale\[data-fastlectures-page-scale="110"\][\s\S]*?--fastlectures-canvas-page-scale:\s*1\.1[\s\S]*?--fastlectures-canvas-page-viewport-width:\s*90\.9090909090909vw/);
  assert.match(css, /html\.fastlectures-web-page-scale\[data-fastlectures-page-scale="125"\][\s\S]*?--fastlectures-canvas-page-scale:\s*1\.25[\s\S]*?--fastlectures-canvas-page-viewport-width:\s*80vw/);
  assert.match(css, /html\.fastlectures-web-page-scale \{ zoom: var\(--fastlectures-canvas-page-scale\); \}/);
});
