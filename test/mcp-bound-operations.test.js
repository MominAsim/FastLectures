...
test('bound tools exclude lifecycle and reject retired aliases and wrong bindings',async()=>{
 const h=harness(()=>({}));
 for(const name of ['fastlectures_list_canvases','fastlectures_open_canvas','fastlectures_start_session','fastlectures_close_session','fastlectures_read_feedback','fastlectures_read_messages','fastlectures_ack_messages','fastlectures_capture_widget'])assert.ok(!BOUND_CANVAS_TOOL_NAMES.includes(name));
 await assert.rejects(h.run('fastlectures_read_file',{path:'/notes.md',unexpected:true}));
 await assert.rejects(h.run('fastlectures_read_file',{sessionId:'other',path:'/notes.md'}),{code:'session_mismatch'});
...
test('one presentation RPC preserves applied receipt for a failed optional capture and never marks done',async()=>{
 ...
 const h=harness(()=>({artifactId:'chart',objectId:'widget',revision:7,captureFailure:{code,message:'Capture existing artifact after readiness',retryTool:'fastlectures_capture_canvas',retryArguments:{sessionId:'bound',target:'artifact',artifactId:'chart'}}}));
 const r=await h.run('fastlectures_present_widget',widgetArgs);
 ...
 assert.equal((await h.run('fastlectures_present_widget',widgetArgs)).reused,true);assert.equal(h.calls.length,1);
 }
 ...
 await assert.rejects(bad.run('fastlectures_present_widget',widgetArgs),{code:'invalid_browser_result'});
});
test('cancellation and session failures remain errors rather than applied results',async()=>{
 ...
 await assert.rejects(h.run('fastlectures_present_widget',widgetArgs),e=>e===error);assert.equal(h.calls.length,1);
 }
});
test('combined presentation returns pixel evidence, source receipt and completion with detailed timing',async()=>{
 ...
 const r=await h.run('fastlectures_present_widget',{...widgetArgs,output:'detailed',completion});
 ...
 await assert.rejects(h.run('fastlectures_present_widget',{...widgetArgs,html:'changed'}),{code:'REQUEST_ID_CONFLICT'});
 const invalid=harness(()=>({artifactId:'chart',objectId:'widget',revision:7}));
 await assert.rejects(invalid.run('fastlectures_present_widget',widgetArgs),{code:'invalid_capture'});
});
test('draw and plot combine optional capture and retain their bounded validated artifact identities',async()=>{
 for(const [name,kind,extra] of [['fastlectures_draw','drawing',{items:[{id:'n',type:'rect'}]}],['fastlectures_plot','plot',{expression:'x'}]]){
...
test('single patch RPC preserves exact conflict and retry receipts without reading source over the bridge',async()=>{
 ...
 await assert.rejects(h.run('fastlectures_patch_file',{...args,contentHash:'stale'}),{code:'SOURCE_CONFLICT'});assert.equal(h.session.mutationRequests.size,0);
 assert.equal((await h.run('fastlectures_patch_file',args)).applied,true);assert.equal(h.calls.at(-1).args.patch,args.patch);assert.equal(h.calls.at(-1).args.content,undefined);
 const count=h.calls.length;assert.equal((await h.run('fastlectures_patch_file',args)).reused,true);assert.equal(h.calls.length,count);
 await assert.rejects(h.run('fastlectures_patch_file',{...args,patch:args.patch.replace('world','again')}),{code:'REQUEST_ID_CONFLICT'});
});
...
test('image mutations preserve independent request receipts and document binding',async()=>{
 const h=harness(op=>({operation:op,revision:2,documentId:'doc'})),source='fastlectures-asset:'+'a'.repeat(64);
 for(const [name,args] of [['fastlectures_upload_image',{requestId:'u',name:'Image',source}],['fastlectures_place_image',{requestId:'p',source}]]){
 assert.equal((await h.run(name,args)).operation,name.replace('fastlectures_','mcp_'));assert.equal((await h.run(name,args)).reused,true);
 await assert.rejects(h.run(name,{...args,source:'fastlectures-asset:'+'b'.repeat(64)}),{code:'REQUEST_ID_CONFLICT'});
 await assert.rejects(h.run(name,{...args,sessionId:'foreign'}),{code:'session_mismatch'});
 }
 await assert.rejects(h.run('fastlectures_edit_canvas',{requestId:'p',action:'show'}),{code:'REQUEST_ID_CONFLICT'});assert.equal(h.calls.length,2);
});
test('inbox preserves two independent pages and acknowledges only explicit IDs',async()=>{
 ...
 const r=await h.run('fastlectures_inbox',{messageAfter:4,feedbackAfter:7});assert.equal(r.messages.nextCursor,5);assert.equal(r.feedback.nextCursor,7);assert.equal(r.image,undefined);assert.equal(h.calls[0].args.capture,false);assert.equal(h.calls[0].args.limit,10);
 assert.deepEqual((await h.run('fastlectures_inbox',{mode:'ack',ids:['m5'],status:'done'})).acknowledged,['m5']);assert.equal(h.calls.length,2);
});
...
test('definite precommit patch failures release unresolved capacity; uncertain outcomes retain IDs',async()=>{
  ...
  for(let i=0;i<5;i++)await assert.rejects(h.run('fastlectures_patch_file',{...base,requestId:`r${i}`}),{code:'PATCH_CONFLICT'});
  assert.equal(h.session.mutationRequests.size,0);
  fail=false;assert.equal((await h.run('fastlectures_patch_file',{...base,requestId:'valid'})).applied,true);
  const unknown=harness(()=>{throw Object.assign(new Error('timeout'),{code:'canvas_timeout'});});
  await assert.rejects(unknown.run('fastlectures_patch_file',{...base,requestId:'unknown'}),{code:'canvas_timeout'});assert.equal(unknown.session.mutationRequests.size,1);
});
...
test('completion flushes queued progress before mutation and synchronizes session state only once',async()=>{
  ...
  const run=()=>executeBoundCanvasTool({name:'fastlectures_edit_canvas',args,session,flushUpdate:async()=>order.push('flush-working'),canvasCall:async()=>{order.push('mutation-done');return {result:{applied:true,completion:args.completion},timing:{requestedAt:1,completedAt:2,durationMs:1}};}});
...
test('presentation presets are normalized once before RPC, without conflicting synthesized dimensions',async()=>{
  ...
  await h.run('fastlectures_plot',{requestId:'plot-size',artifactId:'plot',title:'Plot',expression:'x',presentation:{size:'wide'}});
