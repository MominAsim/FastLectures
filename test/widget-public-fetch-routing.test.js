...
test("Cloud Widget host waits for its authenticated parent's routed public response", async () => {
  ...
  const data = { type:"fastlectures-widget-host-public-fetch-result", requestId:run.messages[0].requestId, status:200,
    headers:{ "content-type":"text/plain", "x-fastlectures-upstream-status":"201" }, body:new TextEncoder().encode("cloud bytes").buffer };
...
test("Local and viewer Widget host retains its existing local public proxy contract", async () => {
  ...
  assert.equal(run.calls[0].options.headers["X-FastLectures-Session"], "session");
