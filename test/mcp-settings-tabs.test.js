...
function setup(runtime,online=false){
  ...
  const window=new EventTarget();window.FASTLECTURES_CONFIG={runtime};window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceOnline:online};let options,lastDevice,refreshes=0;
  window.FastLecturesCloudSettings={api:async()=>({}),origin:()=> 'https://fastlectures.test',signIn:()=>{},signInState:()=>({})};
  window.FastLecturesCloudMcp={mount:(_root,value)=>{options=value;return {refresh:async()=>{refreshes++;},setLanguage(){},updateConnection(){},updateDeviceStatus:value=>{lastDevice=value;}};}};
...
test('Cloud hides Local when no device is online and keyboard never selects a hidden tab',()=>{
  const h=setup('cloud');assert.equal(h.node('mcpLocalTab').hidden,true);h.window.FastLecturesMcpSettings.select('local');assert.equal(h.node('mcpLocalPanel').hidden,true);
...
test('device presence changes reveal Local, then safely return an offline selection to Cloud without refetching settings',()=>{
  const h=setup('cloud');h.window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceOnline:true};h.window.dispatchEvent(new Event('fastlectures:remote-cloud-status'));
  assert.equal(h.node('mcpLocalTab').hidden,false);h.window.FastLecturesMcpSettings.select('local');h.node('mcpLocalTab').focus();
  h.window.FASTLECTURES_REMOTE_CLOUD_STATUS={deviceOnline:false};h.window.dispatchEvent(new Event('fastlectures:remote-cloud-status'));
...
test('local Canvas preserves both existing tabs and enable action, and supplies offline status to its warning',async()=>{
  const h=setup('local');assert.equal(h.node('mcpLocalTab').hidden,false);h.window.FastLecturesMcpSettings.open();assert.equal(typeof h.options.enable,'function');
  h.window.FastLecturesMcpSettings.setDeviceStatus({connected:false});assert.equal(h.lastDevice.connected,false);
  h.window.FastLecturesMcpSettings.select('local');assert.equal(h.node('mcpLocalPanel').hidden,false);
});
test('opening MCP settings after a language change refreshes both tab labels',()=>{
  const h=setup('local');h.document.documentElement.lang='zh-CN';h.window.FastLecturesMcpSettings.open();
