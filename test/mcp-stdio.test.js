...
const { PROTOCOL_VERSION, FastLecturesStdioServer } = require("../src/server/mcp/stdio.js");
...
test("stdio MCP negotiates 2025-11-25, lists tools, emits image blocks, and keeps one owner id", async t => {
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    assert.equal(req.headers["x-fastlectures-mcp-instance"], instanceId);
    const chunks = [];
    req.on("data", chunk => chunks.push(chunk));
    req.on("end", () => {
      const body = JSON.parse(Buffer.concat(chunks).toString("utf8"));
      owners.push(body.ownerId);
      if (body.name === "fastlectures_inspect_session") return;
      if (body.name === "fastlectures_inbox" && body.arguments.mode === "ack") {
      ...
      if (body.name === "fastlectures_start_session") result = {sessionId:"session-a",boardObjectId:null,revision:1};
      if (body.name === "fastlectures_capture_canvas" && body.arguments.target === "artifact") result = { sessionId:"session-a", artifactId:"chart",requestId:"chart", image:{mimeType:"image/png",data:"AQID",bytes:3},width:10,height:20 };
      if (body.name === "fastlectures_capture_canvas" && body.arguments.target !== "artifact") result = { sessionId:"session-a", target:"viewport", image:{mimeType:"image/webp",data:"AQIDBA==",bytes:4},pixelVerified:true,width:24,height:12,encodedBytes:4,revision:7 };
      if (body.name === "fastlectures_present_widget") result = { sessionId:"session-a", artifactId:"combined",requestId:"combined", objectId:"object-a", image:{mimeType:"image/png",data:"BAUG",bytes:3},width:30,height:40 };
      if (body.name === "fastlectures_draw") result = { sessionId:"session-a", artifactId:"drawing",requestId:"drawing", objectId:"draw-a", objectIds:["draw-a"], kind:"drawing", applied:true, pixelVerified:true, image:{mimeType:"image/webp",data:"CgsM",bytes:3},width:16,height:12 };
      if (body.name === "fastlectures_plot") result = { sessionId:"session-a", artifactId:"plot",requestId:"plot", objectId:"plot-a", objectIds:["plot-a"], kind:"plot", applied:true, pixelVerified:false };
      if (body.name === "fastlectures_inbox" && body.arguments.mode !== "ack") result = { sessionId:"session-a", messages:{messages:[],after:0,nextCursor:0,latestCursor:0,hasMore:false},feedback:{after:0,nextCursor:1,latestCursor:1,hasMore:false,truncated:false,entries:[{cursor:1,kind:"text",text:"Feedback"}]}, image:{mimeType:"image/webp",data:"BwgJ",bytes:3},pixelVerified:true,width:12,height:8 };
  ...
  const stdio = new FastLecturesStdioServer({ input, output, record:{ instanceId, secret, port:address.port, host:"127.0.0.1" } }).start();
  t.after(async () => {
    stdio.close();
    input.end();
  ...
  assert.ok(listedTools.result.tools.find(tool=>tool.name==="fastlectures_inbox"));
  const widgetPresentation = listedTools.result.tools.find(tool => tool.name === "fastlectures_present_widget").inputSchema.properties.presentation;
  assert.deepEqual(widgetPresentation.properties.size.enum, ["base","wide","tall","large","page"]);
  assert.equal(listedTools.result.tools.find(tool => tool.name === "fastlectures_present_widget").inputSchema.properties.capture.default, false);
  ...
  assert.deepEqual(prompts.result.prompts.map(prompt => prompt.name), ["fastlectures_visual_explorer","fastlectures_explain_selection","fastlectures_revise_feedback","fastlectures_resume_document"]);
  send({jsonrpc:"2.0",id:15,method:"prompts/get",params:{name:"fastlectures_visual_explorer",arguments:{}}});
  ...
  assert.match(visualPrompt, /fastlectures_present_widget/);
  ...
  assert.match(listedTools.result.tools.find(t=>t.name==="fastlectures_start_session").description,/target:current binds the visible document/);
  assert.deepEqual(listedTools.result.tools.find(t=>t.name==="fastlectures_start_session").inputSchema.properties.target.enum,["current"]);
  assert.match(initialized.result.instructions,/stable sessionKey per conversation/);
  send({jsonrpc:"2.0",id:17,method:"prompts/get",params:{name:"fastlectures_explain_selection",arguments:{}}});
  ...
  send({jsonrpc:"2.0",id:16,method:"prompts/get",params:{name:"fastlectures_visual_explorer",arguments:{unsupported:"value"}}});
  assert.equal((await next()).error.code, -32602);
  send({jsonrpc:"2.0",id:12,method:"prompts/get",params:{name:"fastlectures_resume_document",arguments:{documentId:"document-a"}}});
  assert.match((await next()).result.messages[0].content.text, /show:false/);
  send({jsonrpc:"2.0",id:3,method:"tools/call",params:{name:"fastlectures_list_canvases",arguments:{}}});
  const canvases = await next();
  assert.equal(canvases.result.structuredContent.canvases[0].canvasId, "canvas-a");
  send({jsonrpc:"2.0",id:4,method:"tools/call",params:{name:"fastlectures_start_session",arguments:{canvasId:"canvas-a",instanceId,title:"Work"}}});
  ...
  send({jsonrpc:"2.0",id:5,method:"tools/call",params:{name:"fastlectures_capture_canvas",arguments:{sessionId:"session-a",target:"artifact",artifactId:"chart",quality:"detail"}}});
  ...
  send({jsonrpc:"2.0",id:14,method:"tools/call",params:{name:"fastlectures_capture_canvas",arguments:{sessionId:"session-a"}}});
  ...
  send({jsonrpc:"2.0",id:6,method:"tools/call",params:{name:"fastlectures_present_widget",arguments:{sessionId:"session-a",artifactId:"combined",requestId:"combined",title:"Combined",html:"<p>Combined</p>",capture:true,quality:"basic"}}});
  ...
  send({jsonrpc:"2.0",id:9,method:"tools/call",params:{name:"fastlectures_draw",arguments:{sessionId:"session-a",artifactId:"drawing",requestId:"drawing",title:"Drawing",items:[{id:"n",type:"rect"}],capture:true}}});
  ...
  send({jsonrpc:"2.0",id:10,method:"tools/call",params:{name:"fastlectures_plot",arguments:{sessionId:"session-a",artifactId:"plot",requestId:"plot",title:"Plot",expression:"x*x"}}});
  ...
  send({jsonrpc:"2.0",id:7,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a",capture:true}}});
  ...
  send({jsonrpc:"2.0",id:13,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a",mode:"ack",ids:["request-a"],status:"working"}}});
  ...
  send({jsonrpc:"2.0",id:8,method:"tools/call",params:{name:"fastlectures_inspect_session",arguments:{sessionId:"session-a"}}});
...
test("stdio keeps pending updates compact and acknowledges pull messages directly", async t => {
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      let result = { instanceId, canvases:[{canvasId:"canvas-a",instanceId,title:"Board"}] };
      if (body.name === "fastlectures_start_session") result = {sessionId:"session-a",boardObjectId:null,revision:1};
      if (body.name === "fastlectures_update_session") result = {accepted:true,applied:false,pixelVerified:false,queuedAt:123,sessionId:"session-a"};
      if (body.name === "fastlectures_present_widget") result = {sessionId:"session-a",artifactId:args.artifactId,objectId:"object-a",applied:true,pixelVerified:false};
      if (body.name === "fastlectures_inbox" && body.arguments.mode !== "ack") {
      ...
      if (body.name === "fastlectures_inbox" && body.arguments.mode === "ack") result = {sessionId:"session-a",acknowledged:args.ids,status:args.status};
      if(body.name==="fastlectures_inbox"&&body.arguments.mode!=="ack")result={sessionId:"session-a",messages:result,feedback:{after:0,nextCursor:0,latestCursor:0,hasMore:false,truncated:false,entries:[]}};
      const bytes = Buffer.from(JSON.stringify({result}));
      res.writeHead(200,{"content-type":"application/json","content-length":bytes.length}).end(bytes);
    });
  });
  ...
  send({jsonrpc:"2.0",id:1,method:"initialize",params:{protocolVersion:PROTOCOL_VERSION,capabilities:{},clientInfo:{name:"test",version:"1"}}});
  await next();
  send({jsonrpc:"2.0",id:2,method:"tools/call",params:{name:"fastlectures_list_canvases",arguments:{}}});
  await next();
  send({jsonrpc:"2.0",id:3,method:"tools/call",params:{name:"fastlectures_start_session",arguments:{canvasId:"canvas-a",instanceId,title:"Compact update"}}});
  assert.equal((await next()).result.structuredContent.sessionId,"session-a");

  send({jsonrpc:"2.0",id:4,method:"tools/call",params:{name:"fastlectures_update_session",arguments:{sessionId:"session-a",status:"working",summary:"Queued"}}});
  ...
  assert.deepEqual(bodies.find(body => body.name === "fastlectures_update_session").arguments,{sessionId:"session-a",status:"working",summary:"Queued"});

  send({jsonrpc:"2.0",id:5,method:"tools/call",params:{name:"fastlectures_present_widget",arguments:{sessionId:"session-a",artifactId:"default-capture",requestId:"default-capture",title:"Preview",html:"<p>Preview</p>"}}});
  ...
  assert.equal(bodies.find(body => body.name === "fastlectures_present_widget").arguments.capture,undefined);

  send({jsonrpc:"2.0",id:6,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a"}}});
  const doneMessage = await next();
  send({jsonrpc:"2.0",id:7,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a",mode:"ack",ids:[doneMessage.result.structuredContent.messages.messages[0].id],status:"done"}}});
  assert.deepEqual((await next()).result.structuredContent,{sessionId:"session-a",acknowledged:["request-done"],status:"done"});

  send({jsonrpc:"2.0",id:8,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a",messageAfter:1}}});
  const errorMessage = await next();
  send({jsonrpc:"2.0",id:9,method:"tools/call",params:{name:"fastlectures_inbox",arguments:{sessionId:"session-a",mode:"ack",ids:[errorMessage.result.structuredContent.messages.messages[0].id],status:"error",message:"Could not continue"}}});
...
test("stdio framing rejects malformed JSON without writing logs around protocol messages", async () => {
  ...
  const stdio = new FastLecturesStdioServer({ input, output, record:{instanceId:crypto.randomUUID(),secret:crypto.randomBytes(32).toString("hex"),port:1,host:"127.0.0.1"} }).start();
  input.write("{broken}\n");
...
test("stdio starts before FastLectures, discovers multiple live instances, and pins sessions to the selected instance", async () => {
  const stateDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "fastlectures-stdio-discovery-"));
  const input = new PassThrough(), output = new PassThrough(), next = outputReader(output);
  const stdio = new FastLecturesStdioServer({ input, output, stateDirectory, registryStateDirectory:stateDirectory }).start();
  const send = value => input.write(`${JSON.stringify(value)}\n`);
  send({jsonrpc:"2.0",id:1,method:"initialize",params:{protocolVersion:PROTOCOL_VERSION,capabilities:{},clientInfo:{name:"test",version:"1"}}});
  ...
  for (let index = 0; index < 2; index++) {
    ...
    const server = http.createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        if (body.name === "fastlectures_start_session") result = {sessionId:`session-${index}`,boardObjectId:`object-${index}`,revision:1};
        if (body.name === "fastlectures_update_session") result = {accepted:true,applied:false,pixelVerified:false};
        if(body.name==="fastlectures_inbox"&&body.arguments.mode!=="ack")result={sessionId:"session-a",messages:result,feedback:{after:0,nextCursor:0,latestCursor:0,hasMore:false,truncated:false,entries:[]}};
      const bytes = Buffer.from(JSON.stringify({result}));
        res.writeHead(200,{"content-type":"application/json","content-length":bytes.length}).end(bytes);
      });
  ...
  send({jsonrpc:"2.0",id:4,method:"tools/call",params:{name:"fastlectures_start_session",arguments:{canvasId:"canvas-1",instanceId:records[1].instanceId,title:"Pinned"}}});
  assert.equal((await next()).result.structuredContent.sessionId, "session-1");
  send({jsonrpc:"2.0",id:5,method:"tools/call",params:{name:"fastlectures_update_session",arguments:{sessionId:"session-1",summary:"Exact instance"}}});
  assert.equal((await next()).result.structuredContent.applied, false);
  assert.equal(requests[0].some(item => item.name === "fastlectures_update_session"), false);
  assert.equal(requests[1].some(item => item.name === "fastlectures_update_session"), true);
  stdio.close();
  input.end();
...
test("fastlectures mcp enters stdio directly without an app banner or model preflight", {timeout:5_000}, async () => {
  assert.match(require("../src/cli/main.js").helpText(), /fastlectures mcp connect --host-id ID/);
  const stateDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "fastlectures-cli-mcp-"));
  ...
  try {
    ...
    assert.equal(tools.result.tools.some(tool => tool.name === "fastlectures_list_canvases"), true);
    child.stdin.write(`${JSON.stringify({jsonrpc:"2.0",id:3,method:"tools/call",params:{name:"fastlectures_list_canvases",arguments:{}}})}\n`);
    assert.deepEqual((await next()).result.structuredContent, {canvases:[],discovery:{status:"no-local-instance",instances:0,reachable:0}});
    child.stdin.end();
...
test("canvas discovery distinguishes empty, unavailable, partial and invalid instances without leaking diagnostics", async t => {
  ...
  const stdio = new FastLecturesStdioServer();
  let selected = [];
