...
(() => {
  const COPY = {
    en: {
      localCanvas:"Local Canvas", language:"Language", kicker:"LOCAL ACCESS", checkingTitle:"Checking this FastLectures server", checkingBody:"Confirming how this Canvas is protected.", checking:"Checking access...",
      setupTitle:"Protect this FastLectures server", setupBody:"Choose how every browser and device may access this running FastLectures instance.", scopeTitle:"Instance-wide protection", scopeBody:"This is not a code for only this browser. Once set, the same code is required by every browser and device that opens this running FastLectures instance. It helps prevent unauthorized people on your local network from accessing the Canvas.", setCode:"Set a 6-digit security code", setCodeHelp:"One shared code will protect this server from every new local-network visitor.", continueOpen:"Continue without a code", continueOpenHelp:"Anyone on this local network may be able to open this Canvas.",
      forgotLead:"A forgotten code is not a problem.", forgotCode:"Forgot the code?", restartHelp:"Restart FastLectures to clear it. Your Canvas files and settings are not affected.", enterTitle:"Enter your security code", enterBody:"Use the shared code for this running FastLectures server.", createTitle:"Create a security code", createBody:"Choose six digits for every device that opens this FastLectures server. The code is submitted as soon as the sixth digit is entered.", unlockStep:"6-digit code",
      ...
      wrong:"That security code is not correct.", tooMany:"Too many attempts. Try again in {seconds} seconds.", failed:"FastLectures could not update local access. Try again.", unavailableTitle:"This FastLectures server is unavailable", unavailableBody:"Check the local connection, then try again.", retry:"Retry",
    },
    zh: {
      localCanvas:"本地画布", language:"语言", kicker:"本地访问", checkingTitle:"正在检查这台 FastLectures", checkingBody:"正在确认这张画布采用的访问保护方式。", checking:"正在检查访问状态…",
      setupTitle:"保护这台 FastLectures 服务器", setupBody:"请选择所有浏览器和设备访问当前运行中 FastLectures 实例的方式。", scopeTitle:"实例级保护", scopeBody:"这不是仅针对当前浏览器的安全码。设置后，所有访问当前运行中 FastLectures 实例的浏览器和设备都必须使用同一个安全码，用于防止局域网内未经许可的人访问画布。", setCode:"设置 6 位安全码", setCodeHelp:"这是当前实例共用的安全码，所有新的局域网访问者都需要输入。", continueOpen:"不设置安全码", continueOpenHelp:"同一局域网中的任何设备都可能直接打开这张画布。",
      forgotLead:"忘记安全码也没关系。", forgotCode:"忘记安全码？", restartHelp:"重新启动 FastLectures 即可清除。画布文件和设置不会受到影响。", enterTitle:"输入安全码", enterBody:"请输入当前运行中 FastLectures 实例共用的安全码。", createTitle:"创建安全码", createBody:"请为所有访问这台 FastLectures 的设备设置 6 位数字。输入第六位后会立即提交。", unlockStep:"6 位安全码",
      back:"返回", clear:"清除", openRiskTitle:"这张画布将向局域网开放", openRiskBody:"同一网络中的其他人可能可以打开画布、使用已配置的 AI 服务，并管理本地插件。", confirmOpen:"本次在局域网保持开放",
      wrong:"安全码不正确。", tooMany:"尝试次数过多，请在 {seconds} 秒后重试。", failed:"FastLectures 无法更新本地访问设置，请重试。", unavailableTitle:"无法连接这台 FastLectures", unavailableBody:"请检查本地连接后重试。", retry:"重试",
    },
  };
  ...
  let language=localStorage.getItem("fastlectures-language")==="zh"?"zh":"en",flow="unlock",entry="",busy=false,cooldownTimer=0,cooldownUntil=0;

  function applyAppearance() {
    const storedTheme=localStorage.getItem("fastlectures-theme")||localStorage.getItem("ghostboard-theme"),
      storedPalette=localStorage.getItem("fastlectures-studio-palette"),
    ...
    if(removedTheme){localStorage.setItem("fastlectures-theme","studio");localStorage.setItem("fastlectures-studio-palette",palette);}
  ...
  async function request(path,payload) {
    ...
    if(typeof body.accessSessionToken==="string"&&body.accessSessionToken)sessionStorage.setItem("fastlectures-access-session",body.accessSessionToken);
    return body;
  }
  ...
  async function loadStatus() {
    ...
    try {
      ...
      if(typeof status.accessSessionToken==="string"&&status.accessSessionToken)sessionStorage.setItem("fastlectures-access-session",status.accessSessionToken);
  ...
  document.querySelectorAll("[data-language]").forEach(button=>button.addEventListener("click",()=>{language=button.dataset.language;localStorage.setItem("fastlectures-language",language);applyLanguage();}));
  ...
  window.addEventListener?.("storage",event=>{if(event.key==="fastlectures-theme"||event.key==="fastlectures-studio-palette")applyAppearance();});
