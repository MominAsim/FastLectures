...
function getAuthoringGuidance(id, detail = "brief") {
  ...
  const document = {
    "visual-explorer":`Create an explanation the user can understand and use. Lead with the answer or central relationship. Choose the smallest useful format: labeled flow for a process, a table for comparison, a timeline for sequence, or a local interactive model for cause and effect. Do not turn a requested product UI into an explanatory dashboard; use general-html for a real page. Existing edits retain their source, style and artifact identity.

Use one complete HTML/CSS/SVG document with minimal JavaScript. Prefer a clear title, a short explanation, and one meaningful visual. Show units, assumptions, uncertainty and source attribution next to the relevant data. Do not invent measurements, quotations or research findings. Use restrained color consistently, readable normal CSS type, labeled diagrams and enough whitespace to distinguish groups. A visual must explain a relationship rather than repeat nearby prose.

Build responsive content: root width:100%, min-width:0, border-box sizing; use media/container queries and vertical scrolling for overflow. Never scale a whole page down with transform/zoom. Default/page uses the available unobscured viewport; requested dimensions are preferences capped independently. Use the returned viewport as the actual CSS size. Do not guess Canvas world coordinates. The host places independent content; relativeTo refers to a known artifactId. Updates preserve the user's current position and size.

Use native, keyboard-accessible controls with labels and visible state. Sliders, tabs, filters and buttons update locally without model calls. Every control must produce the promised result. Only an explicit AI request uses data-penecho-action="choose" plus a bounded data-penecho-prompt; label it as a request. Keep formulas, derived values and visual state consistent, including reset and boundary cases. Prefer HTML/SVG for simple explanations; load a science supplement only when calibrated geometry or simulation requires its renderer contract.

Present with a stable artifactId and unique requestId. Keep HTML elements, CSS declarations and JavaScript statements on separate stable lines for later diffs. Authorized uploaded images use immutable penecho-asset references; host filesystem paths do not render. After initial render or meaningful layout changes, notify the parent with {type:"penecho-widget-updated"}, never every frame. Read the returned sourcePath before patching, and use its contentHash. Keep new user geometry and unrelated content.

Capture only when unresolved pixels matter, preferably on the final mutation with completion. Inspect available image evidence; a successful capture alone is not proof of visual correctness. Fix concrete missing requirements, wrong data, broken controls or unreadable content. Stop when correct and usable. Full guidance is available for specialized examples; ordinary HTML/SVG explanations can proceed from this brief.`,
