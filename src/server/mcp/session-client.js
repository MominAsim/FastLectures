...
function httpRequest(url, credentials, message, session, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on('socket',socket=>socket.once('secureConnect',()=>{clearTimeout(setupTimer);dispatched=true;}));
...
function createSessionClient(options = {}) {
  ...
  const starts=new Map(),lifetime=new AbortController();
  ...
  async function send(message, sid=session, extra={}){return request(endpoint,credentials,message,sid,{protocolVersion,signal:lifetime.signal,...extra});}
  ...
  async function ensure(){
    if(session)return;if(initFlight)return initFlight;
  ...
  async function restore(message){
    ...
    saved.current=result.sessionId;saved.owner=session;
    return {...message,params:{...message.params,arguments:{...args,sessionId:result.sessionId}}};
  }
  async function execute(message){
    ...
    for(let attempt=0;attempt<3;attempt++){
      try{response=await send(await restore(message));}catch(e){
        if(e.transport||typeof e.dispatched==='boolean'){session=undefined;if(e.dispatched!==false)throw Error('PenEcho connection failed; request may have completed. Inspect Canvas before retrying: '+e.message);if(attempt===2)throw e;await ensure();continue;}
        if(e.status===404&&attempt<2){session=undefined;await ensure();continue;}throw e;
      }
      if(response.status===404){session=undefined;if(attempt<2){await ensure();continue;}}
      checked(response);
  ...
  function accept(message){
    ...
    const run=async()=>{try{if(closed)return;const response=await execute(message);if(Object.hasOwn(message,'id')&&response)write(response);}catch(e){if(Object.hasOwn(message,'id'))write({jsonrpc:'2.0',id:message.id,error:{code:-32000,message:safeMessage(e)}});}finally{pending--;if(activity)lastActivity=now();}};
    // Cancellation must be able to interrupt the currently executing request.
    if(message.method==='notifications/cancelled'&&session)void run();else queue=queue.then(run,run);
  }
...
async function main(argv=process.argv.slice(2)){
  ...
  try{for(let i=0;i<argv.length;i++){const flag=argv[i];if(flag==='--help'){process.stdout.write('PenEcho per-session stdio HTTPS bridge\n--host-id ID [--state-directory DIR] [--idle-timeout-ms 1800000] [--idle-exit-ms MS]\nCanvas recovery across process restarts requires the client to retain sessionKey and documentId.\n');return 0;}const key={'--host-id':'hostId','--state-directory':'stateDirectory','--idle-timeout-ms':'idleTimeoutMs','--idle-exit-ms':'idleExitMs'}[flag];if(!key||!argv[i+1])throw Error('Unknown or incomplete option');options[key]=key.endsWith('Ms')?Number(argv[++i]):argv[++i];if(key.endsWith('Ms')&&(!Number.isFinite(options[key])||options[key]<=0))throw Error('Idle timeout must be positive');}if(!/^[a-f0-9]{64}$/i.test(options.hostId||''))throw Error('--host-id is required');const client=createSessionClient(options);for(const signal of ['SIGTERM','SIGINT','SIGHUP'])process.once(signal,()=>void client.close());return 0;}catch(e){process.stderr.write(e.message+'\n');return 1;}
}
module.exports={createSessionClient,httpRequest,main};
