...
export function parseClientEnvelope(raw) {
  ...
  try {
    ...
    throw new Error('FastLectures Agent messages must be valid JSON.')
  }
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('FastLectures Agent message envelope is invalid.')
  }
  if (value.version !== CANVAS_AGENT_PROTOCOL_VERSION || !CLIENT_MESSAGE_TYPES.has(value.type)) {
    throw new Error('FastLectures Agent protocol version or message type is unsupported.')
  }
  if (!Number.isSafeInteger(value.seq) || value.seq < 1 || value.seq > Number.MAX_SAFE_INTEGER) {
    throw new Error('FastLectures Agent message sequence is invalid.')
  }
